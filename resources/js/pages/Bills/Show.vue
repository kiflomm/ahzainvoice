<template>
  <AppLayout :title="`Bill #${bill.record_number}`">
    <template #header>
      <div class="flex justify-between items-center">
        <div class="flex items-center space-x-4">
          <Button variant="outline" asChild>
            <Link :href="route('bills.index')" class="flex items-center">
              <ArrowLeft class="h-4 w-4 mr-2" />
              Back to Bills
            </Link>
          </Button>
          <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Bill #{{ bill.record_number }}
          </h2>
        </div>
          ...
          <div class="p-6 text-gray-900">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-900">{{ bill.start_date }}</p>
              </div>

              <div>
                <h4 class="text-sm font-medium text-gray-500">Due Date</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.end_date }}</p>
              </div>

              <!-- Purchase Type -->
              <div>
                <h4 class="text-sm font-medium text-gray-500">Purchase Type</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.purchase_type }}</p>
              </div>

              <!-- Unit Details -->
              <div>
                <h4 class="text-sm font-medium text-gray-500">Unit</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.unit }}</p>
              </div>

              <!-- Quantity -->
              <div>
                <h4 class="text-sm font-medium text-gray-500">Quantity</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.quantity }}</p>
              </div>

              <!-- Unit Price -->
              <div>
                <h4 class="text-sm font-medium text-gray-500">Unit Price</h4>
                <p class="mt-1 text-sm text-gray-900">{{ formatCurrency(bill.unit_price) }}</p>
              </div>

              <!-- VAT -->
              <div>
                <h4 class="text-sm font-medium text-gray-500">VAT Rate</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.vat }}%</p>
              </div>
                ...
                <div class="bg-gray-50 p-4 rounded-md">
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <p class="text-lg font-semibold">{{ formatCurrency(bill.value) }}</p>
                    </div>
                    <div>
                      <span class="text-sm text-gray-500">VAT Amount:</span>
                      <p class="text-lg font-semibold">{{ formatCurrency(bill.value_after_vat - bill.value) }}</p>
                    </div>
                    <div>
                      <span class="text-sm text-gray-500">Total:</span>
                      <p class="text-lg font-semibold">{{ formatCurrency(bill.value_after_vat) }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="col-span-2">
                ...
                <p class="mt-1 text-sm text-gray-900">{{ bill.description || 'No description provided' }}</p>
              </div>

              <!-- Reference Numbers -->
              <div>
                <h4 class="text-sm font-medium text-gray-500">MRC Number</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.mrc_number || 'N/A' }}</p>
              </div>

              <div>
                <h4 class="text-sm font-medium text-gray-500">CDN Number</h4>
                <p class="mt-1 text-sm text-gray-900">{{ bill.cdn_number || 'N/A' }}</p>
              </div>
            </div>
          </div>
        </div>
